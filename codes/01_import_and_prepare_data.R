...
data_tot <- merge(data_claims, data_policy, by = c("id_policy","id_client","id_vehicle","id_year"),
                  all = T) %>% 
  mutate(claim_nb = ifelse(is.na(claim_nb), 0, claim_nb),
         claim_amount = ifelse(is.na(claim_amount), 0, claim_amount)) 
...
# On sauvegarder la base obtenue ------------------------------------------

saveRDS(data_claims,"./data/data_claims_ecrete_year0.rds")


# Préparation donnees YEAR 1 ----------------------------------------------

# Création d'une variable numero du departement ------------------------------
...
# Ajout de la variable densite de la population par departement -----------
...
# Variable categorielle de pol_bonus --------------------------------------
